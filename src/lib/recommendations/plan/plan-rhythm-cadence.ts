...
export function classifyRhythmCadence(
  text: string,
  typeOrRole: RecommendationType | string,
): RhythmCadence {
  ...
  if (typeOrRole === "recovery_rule" || typeOrRole === "environment_change") return "either";
  if (typeOrRole === "dont") return "either";
  return "either";
}

export function isValidAnchorCandidate(row: Pick<RecommendationRow, "type" | "recommendationRole" | "text">): boolean {
  if (row.type === "dont" || row.type === "mindset_shift" || rowHasRole(row, "coach_note")) return false;
  if (ANCHOR_DISQUALIFY_PATTERN.test(row.text)) return false;
  if (rowHasRole(row, "recovery_rule") && ANCHOR_DISQUALIFY_PATTERN.test(row.text)) return false;
  return (
...
export function anchorCandidateRank(
  row: Pick<RecommendationRow, "type" | "recommendationRole" | "text">,
): number {
  if (!isValidAnchorCandidate(row)) return 99;
  if (rowHasRole(row, "first_action")) return 0;
  ...
  if (rowHasRole(row, "environment_change")) return 3;
  return 4;
}
