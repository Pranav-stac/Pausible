...
function deterministicIntegratedPlan(planOutput: PlanOutput): IntegratedPlanSynthesis {
  return {
...
function capPlanNotes(notes: string[]): string[] {
...
function mergeParsedPlan(
  ...
  const fallback = deterministicIntegratedPlan(planOutput);
  ...
  const raw = {
    ...
    phases,
    plan_notes: capPlanNotes(parsed?.plan_notes ?? []),
  };
...
export async function synthesizeIntegratedPlanPage(
  ...
  const userPrompt = buildIntegratedPlanPrompt(planOutput, profile, secondaryBlendPct);
  ...
  if (!apiKey) {
    return { ...fallback, synthesisError: "LLM API key not configured — showing deterministic plan copy." };
  }
  ...
  return mergeParsedPlan(planOutput, parsed, true, null);
}
