...
export function CheckoutClient({ bootstrapPriceInr }: { bootstrapPriceInr: number }) {
  ...
  if (!ready) {
    return <div className={`px-1.5 py-16 text-center ${APP_BODY} sm:px-2`}>Preparing checkout…</div>;
  }

  if (!attemptId) {
    return (
      <div className="mx-auto max-w-lg px-1.5 py-16 text-center sm:px-2">
        <p className={APP_BODY}>Missing attempt. Start from the assessment.</p>
        <Link href="/assessment/default" className={`mt-4 inline-block font-semibold ${APP_LINK_BACK}`}>
          Go to assessment
        </Link>
      </div>
    );
  ...
  if (isFirebaseConfigured() && !hasGoogleIdentity) {
    return (
      <div className={`mx-auto max-w-lg px-4 py-20 text-center ${APP_PAGE_BG_SOFT}`}>
        ...
        <h1 className={`mt-4 ${APP_HEADING_MD}`}>Sign in to continue checkout</h1>
        ...
        <button
          ...
          className={`mt-8 ${CTA_PRIMARY_CLASS}`}
        >
          Sign in with Google
        </button>
      </div>
  ...
  return (
    <div className={`${APP_PAGE_BG_SOFT} px-1.5 py-10 sm:px-2 sm:py-12`}>
      <div className="mx-auto max-w-3xl">
        <Link href="/" className="inline-flex items-center gap-2 font-semibold text-[#0D1B2A]" aria-label="Home">
          <span aria-hidden className="text-[#6E7191]">
            ←
          </span>
          <BrandLogo sizeClass="text-base" />
        </Link>
        <h1 className={`mt-6 ${APP_HEADING_MD}`}>Checkout</h1>
        <p className={`mt-2 ${APP_BODY}`}>
          Unlock the full premium breakdown for this attempt. Retakes require a new payment each time.
        </p>

        <div className={`mt-10 ${FORM_CARD_CLASS} p-6`}>
          <div className="flex items-baseline justify-between gap-4">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide text-[#6E7191]">Due today</div>
              <div className="mt-2 text-3xl font-bold text-[#0D1B2A]">{formatInr(amount)}</div>
              {percentOff > 0 && (
            ...
            <div className="text-right text-xs text-[#6E7191]">
              Attempt{" "}
              <span className="font-mono text-[11px] text-[#4D4D4D]">{attemptId.slice(0, 8)}…</span>
            </div>
          </div>

          <label className={`mt-8 block ${INPUT_LABEL}`}>
        ...
        <div className="mt-10 space-y-4">
          <div className="grid gap-3 sm:grid-cols-3">
            <button
              ...
              className={`${CTA_PRIMARY_CLASS} disabled:opacity-50`}
            >
            ...
            <button
              ...
              className={`${CTA_SECONDARY_CLASS} disabled:opacity-50`}
            >
